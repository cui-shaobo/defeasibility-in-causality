...
def count_accuracy(result_file, original_score_col, updated_score_col1, updated_score_col2,
    ...
    if updated_type == 'assumption':
        return original_scores, updated_scores, np.greater_equal(updated_scores, original_scores).sum() / len(updated_scores)
    elif updated_type == 'defeater':
        return original_scores, updated_scores, np.greater_equal(original_scores, updated_scores).sum() / len(updated_scores)
    else:
...
if __name__ == '__main__':
    ...
    output_file_assumption = 'CEQ_assumption.csv'

    calculate_ceq_file(filename, cause_col, effect_col, supp_col, original_score_col, updated_score_col1, updated_score_col2, output_file_assumption)

    original_score, updated_score_assumption, acc = count_accuracy(output_file_assumption, original_score_col, updated_score_col1, updated_score_col2,
                                                                    updated_two_cols_strategy='max', updated_type='assumption')
    df_assumption = pandas.DataFrame({
        "origin": original_score,
        "assumption": updated_score_assumption
    })
    df_assumption.to_csv('df_assumption_ceq.csv', index=False)
    ...
    output_file_defeater = 'CEQ_defeater.csv'

    calculate_ceq_file(filename, cause_col, effect_col, supp_col, original_score_col, updated_score_col1, updated_score_col2, output_file_defeater)

    original_score, updated_score_defeater, acc = count_accuracy(output_file_defeater, original_score_col, updated_score_col1, updated_score_col2,
                                                                    updated_two_cols_strategy='max', updated_type='assumption')
    df_defeater = pandas.DataFrame({
        "origin": original_score,
        "defeater": updated_score_defeater
    })
    df_defeater.to_csv('df_defeater_ceq.csv', index=False)