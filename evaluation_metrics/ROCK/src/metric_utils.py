...
def test_eps_conan(func, dat, eps=0.1, col_name='score', n_smp=None, **kwargs):
    def get_probs(s, prefix=""):
        p_xd, p_dy, p_xy = s[['p_xd'+prefix, 'p_dy'+prefix, 'p_xy'+prefix]]
        p_dy = np.array(p_dy)
        p_xd = np.array(p_xd)
        p_xy = np.array(p_xy)
        if n_smp is None:
    ...
    for prefix in ["", "_d", "_a"]:
        dat[col_name+prefix] = dat.apply(
            lambda s : func(*get_probs(s, prefix), eps=eps, **kwargs),
            axis=1
        )



def test_eps_copa(func, dat, copa, eps=0.1, col_name='score', n_smp=None, **kwargs):
    def get_probs(s):
    ...
    dat[col_name] = dat.apply(lambda s : func(*get_probs(s), eps=eps, **kwargs)
                                               , axis=1)
    return np.mean( copa['label'] == dat.groupby(['index']).apply(lambda s: s.iloc[s[col_name].argmax()].label_idx))
...
def gen_eg(ds, eps, spacy_model, file_pref=None):
    ...
    X = [tt_ize(crop_sent(x.replace('$', r'\$'), spacy_model=spacy_model, sent_idx=0,offset=0)) for xidx, x in enumerate(cov)]
...
def ablation_query(dt, D, F, S, Q, C, E):
    return dt.query(f"(normalization == {S}) and (direct_match=={D}) and (use_cooccur=={C}) and (temp_filter=={F}) and (res_norm=={E}) and (pxa_norm=={Q})")
